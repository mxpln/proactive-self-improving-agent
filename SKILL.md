...
version: 1.0.0
description: "Skill for automatically capturing experience and safely evolving. Trigger conditions: (1) When a command/operation fails → Log to ERRORS.md (2) When corrected by the user ('That's wrong'/'It should be') → Log to LEARNINGS.md (3) When the user requests a non-existent capability → Log to FEATURE_REQUESTS.md (4) When external APIs/tools fail → Log in ERRORS.md (5) When discovering outdated/incorrect knowledge → Log in LEARNINGS.md (6) When discovering better practices → Log in LEARNINGS.md (7) Upon completing each task → Review process; log new insights in LEARNINGS.md if applicable. Duplication Prevention Principle: Skip writing if no new insights exist or existing entries cover the topic. Each entry also appends JSONL logs to .learnings/CHANGELOG.md. Insights recurring ≥3 times are promoted to AGENTS.md/TOOLS.md/SOUL.md. See main text for details.
author: yanhongxi-openclaw
---

# Proactive Self-Improving Agent

**Auto-Gain Experience · Secure Evolution · Track Your Progress**

Enable agents to automatically identify errors, corrections, and best practices during daily operations, systematically document them, and securely transform experience into long-term capabilities.
